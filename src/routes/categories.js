...
router.get("/:id", async (req, res) => {
  ...
  try {
    ...
    if (!category)
      return res
        .status(404)
        .json({ error: error.message || "Category not found" });
...
router.post("/", authenticate, async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      return res
        .status(409)
        .json({ message: `A categoria "${description}" já existe.` });
    }

    return res.status(500).json({
      message:
        "Erro interno no servidor ao tentar criar a categoria. Verifique os logs do backend.",
