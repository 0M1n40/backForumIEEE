...
router.get("/", async (req, res) => {
  ...
  try {
    let questions;
    if (titulo) {
      questions = await Question.searchByTitle(titulo);
    } else {
...
router.post("/", authenticate, async (req, res) => {
  ...
  if (!title || !content || !categoryId) {
    return res
      .status(400)
      .json({ message: "Título, conteúdo e ID da categoria são obrigatórios" });
  }
...
router.put("/:id", authenticate, async (req, res) => {
  ...
  try {
    ...
    if (question.usuarioId !== userId) {
      return res
        .status(403)
        .json({ message: "Você não tem autorização para editar esta dúvida" });
    }
    if (!title || !content || !categoryId) {
      return res
        .status(400)
        .json({
          message: "Título, conteúdo e ID da categoria são obrigatórios",
        });
    }
...
router.delete("/:id", authenticate, async (req, res) => {
  ...
  try {
    ...
    if (question.usuarioId !== userId) {
      return res
        .status(403)
        .json({ message: "Você não tem autorização para deletar esta dúvida" });
    }
